...
export const sendUpdatePasswordOtp = () => async (dispatch) => {
  try {
    dispatch({ type: UPDATE_PASSWORD_REQUEST });

    const { data } = await axios.post(`/api/v2/password/update/send-otp`);
...
export const verifyUpdatePasswordOtp = (otpData) => async (dispatch) => {
  try {
    dispatch({ type: UPDATE_PASSWORD_REQUEST });
    ...
    const { data } = await axios.post(
      `/api/v2/password/update/verify-otp`,
      otpData,
      config
    );
